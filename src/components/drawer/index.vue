...
import { computed, nextTick, onMounted, onUnmounted, ref, watch, useSlots } from 'vue'
...
const props = withDefaults(defineProps<Props>(), { width: 16, widthUnit: 'rem' })
...
const computedWidth = computed(() => {
  ...
  } else if (props.width.length === parseInt(props.width, 10).toString().length) {
    return `${props.width}${props.widthUnit}`
  }
  return props.width
})

watch(() => [props.modelValue, mdBreakpoint.value, props.absolute, props.fixed], () => {
  setOverlay();
  refreshLayoutStyle();
});

watch(() => [props.right, props.width], () => {
  refreshLayoutStyle();
});
...
const setOverlay = (): void => {
  if (props.modelValue && absoluteComputed.value) {
    if (!overlayNode) {
      ...
      const appParent: HTMLElement = document.querySelector('#app.e-app') || new HTMLElement();
...
const refreshLayoutStyle = (): void => {
  ...
  const propertyValue =
    (absoluteComputed.value || !props.modelValue) ? '0px' : `${computedWidth.value}`;
  if (eMainNode) {
    if (props.right) {
